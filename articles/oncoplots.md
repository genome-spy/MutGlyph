...
Use `top` for the most frequently altered genes, or provide an explicit
gene list. `keepGeneOrder = TRUE` preserves the supplied order. The same
pattern works for a selected and ordered set of samples.
...
`minMut` provides a concise alternative when the exact genes are not
known. A value below one is treated as a cohort fraction; a value of one
or more is a sample count.
...
Clinical tracks can be categorical or numeric. Each track gets an
independent scale, and categorical tracks use GenomeSpy’s default
categorical palette unless a Vega scheme or explicit mapping is
supplied. The transition/ transversion track is computed from the MAF
variants.
...
Custom bars use a two-column data frame. The first column identifies a
sample or gene; the numeric second column supplies the value and its
name becomes the axis title. A numeric clinical field name can be used
directly for the top bar.
...
Custom top and right data replace their default stacked summaries. A
custom left bar adds a new gene-aligned column. Missing displayed keys
are shown as zero and reported with a warning.

## Include GISTIC calls

When
[`maftools::read.maf()`](https://rdrr.io/pkg/maftools/man/read.maf.html)
is given GISTIC results, MutGlyph adds gene-level `Amp` and `Del` events
to the matrix. Copy-number events occupy half of a gene band so that a
...
Set `includeColBarCN = FALSE` to keep the copy-number layer in the
matrix but limit the top bars to sequence mutations.
