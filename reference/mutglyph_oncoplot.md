...
`mutglyph_oncoplot()` deliberately imitates the layout, ordering rules,
and commonly used argument names of
...
- rowHeight:
  ...
  Height in pixels of each gene row.
...
scales. Sample labels use ranged text and appear only when their bands
are wide enough; zooming in reveals labels hidden in a dense overview.
Custom summary bars accept one numeric metric keyed by sample or gene,
and use the metric's column name as the axis title.
