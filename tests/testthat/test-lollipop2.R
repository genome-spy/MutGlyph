test_that("lollipopPlot2 mirrors two cohorts around one protein model", {
  ...
  expect_identical(spec$title$text, "FLT3 mutation comparison")
  expect_identical(
    spec$title$subtitle,
...
test_that("lollipopPlot2 can suppress its autogenerated title", {
  spec <- lollipopPlot2(
    m1 = data.frame(
      gene = "FLT3",
      position = 599,
...
test_that("lollipopPlot2 validates meaningful inputs", {
  ...
  expect_error(
    lollipopPlot2(mutations, mutations, gene = "FLT3", showTitle = 1),
    "showTitle"
  )
})
